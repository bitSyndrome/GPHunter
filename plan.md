...
* **In (2차 구현 완료):** 💔 아까운 유령(완성도 가중) 정렬, 되살리기/보내주기 액션, **AI 작업 요약(다중 LLM)**, Slack/Discord 주간 알림. → [§2-4](#2-4-2차-구현-기능-mvp-이후) 참고.
* **Out:** 팀/협업 공유, 모바일 앱. (Gemini 등 타 CLI 어댑터는 미구현이나, AI 요약은 공급자 무관 설계로 Gemini 등 연결 가능.)
...
* **수집 에이전트:** `ghost-hunter-hook` (Node 기반 글로벌 CLI 패키지). 동작:
  ...
  2. **프로젝트 식별 키 생성** (다기기 동기화 + 원격 추가 대응 핵심):
     ...
     - **키 정규화(canonical key, 2차):** 수집 단계에서 로컬 키를 정규형으로 변환 — Windows 경로 전체 대소문자 무시 + 구분자 통일 + 끝 슬래시 제거, 호스트명 소문자화(POSIX는 대소문자 유지). 같은 폴더가 케이싱·구분자 차이로 갈라지지 않음. 서버 시작 시 **백필 마이그레이션**으로 정규화 이전 데이터의 중복까지 소급 병합(멱등).
...
| `GET`  | `/api/v1/projects?sort=ghost\|active\|momentum\|regret&archived=false` | 리더보드 리스트(계산된 ghost_tier·momentum·regret_score 포함) |
| `GET`  | `/api/v1/projects/:id` | 상세 + 활동 스파크라인(일별 집계) |
| `PATCH`| `/api/v1/projects/:id` | 수동 보정(`archived`, `pinned`, `completion_pct`, `name`, `description`, `epitaph`) |
| `POST` | `/api/v1/projects/:id/summarize` | **AI 작업 요약** 생성(온디맨드, LLM 미설정 시 503) |
| `GET`  | `/api/v1/stats` | 요약(총 프로젝트 수, 유령 수, 무덤 수 등) |
| `GET·PUT·DELETE` | `/api/v1/notifications` | Slack/Discord 웹훅 설정 조회·저장(채널 자동인식)·삭제 |
| `POST` | `/api/v1/notifications/test` | 다이제스트 즉시 전송(저장 전 미리보기) |
...
* **리더보드 4개 정렬 모드(탭):**
  ...
  * 💔 **Most Regrettable** — `regret_score`(완성도 가중) 내림차순(유령 등급 이상만). *"거의 다 했는데 버려진, 가장 아까운 유령"*. (2차)
...
* **액션:** 핀 고정, **되살리기**(`cd <경로> && claude` 복귀 명령 복사 + 복원), **보내주기**(회고 한 줄 + 안식), 완성도 수동 입력. 상세 패널에서 **AI 기억 보조**(요약 생성). (2차)
...
MVP 이후 추가된 4개 기능 + 중복 인식 근본 수정. 모두 **키/웹훅 없으면 자동 비활성**이라 기본 설치 단순함을 유지.

### a) 💔 아까운 유령 (완성도 가중 랭킹)
유령 점수를 "완성도"로 가중해, 거의 완성됐는데 버려진 후회 큰 유령을 우선 노출.

```
completion  = completion_pct(수동) ?? maturity_score
regret_score = ghost_score × (0.25 + 0.75 × completion/100)
```
* 완성도 100% 유령은 유령점수 그대로, 0%는 1/4로 감쇠 → 완성도가 순위를 재배열하되 완전히 가리지는 않음.
* `regret` 정렬은 유령/무덤 등급만 필터 후 `regret_score` 내림차순.

### b) 되살리기 / 보내주기
* **되살리기:** `cd "<path>" && claude` 복귀 명령을 클립보드로 복사(아카이브였으면 복원·회고 삭제).
* **보내주기:** 회고 한 줄(묘비명, `epitaph`)과 안식 시각(`retired_at`)을 기록하며 아카이브. 안식 카드 하단에 `🪦 회고` 표시.

### c) 🤖 AI 기억 보조 (다중 LLM, 온디맨드)
최근 세션 기록 5개로 **한 줄 요약 + 다음 할 일**을 생성(`{summary, next_step}`).
* **공급자 무관:** OpenAI 호환 `/chat/completions` 한 겹으로 추상화 → `GPH_LLM_BASE_URL`/`GPH_LLM_API_KEY`/`GPH_LLM_MODEL` 설정만으로 Claude·OpenAI·**Gemini**·Ollama 연결. 키 없으면 503.
* **온디맨드:** 상세 패널의 버튼을 누를 때만 호출(비용 최소화). 결과는 `ai_summary`/`ai_next_step`/`ai_model`/`summarized_at`에 캐시.
* 사고(thinking) 모델 대응: `max_tokens` 여유 확보(Gemini 2.5로 실검증).

### d) 🔔 Slack/Discord 주간 다이제스트
홈 헤더 **🔔 알림 설정** 버튼 → 웹훅 URL만 붙여넣으면 채널 자동 인식(호스트 기반).
* 다이제스트: 활성·유령·무덤 수 + 💔 가장 아까운 유령 Top3 + 🔥 이번 주 가장 활발. Slack `text` / Discord `content` 형식.
* 매시간 점검 → 7일 경과 설정에 자동 전송(서버 스케줄러). 설정은 사용자별 DB 저장, "테스트 전송"으로 저장 전 확인.
...
projects(
  ...
  epitaph NULLABLE, retired_at NULLABLE,             -- 보내주기(회고/안식) (2차)
  ai_summary NULLABLE, ai_next_step NULLABLE,        -- AI 기억 보조 (2차)
  ai_model NULLABLE, summarized_at NULLABLE,
...
-- 알림 설정 (Slack/Discord 웹훅, 사용자별 1행) (2차)
notification_config(
  user_id PK FK, webhook_url, kind,        -- kind: 'slack' | 'discord'
  enabled BOOL, last_sent_at NULLABLE
)
```

* `ghost_tier`, `ghost_score`, `momentum`, `regret_score`는 조회 시 계산(또는 일별 집계 캐시 테이블 `daily_activity`).
...
* **5단계: 2차 고도화 (구현 완료)** — 키 정규화(canonical key)+백필 마이그레이션으로 중복 인식 근본 수정 · 💔 아까운 유령 정렬 · 되살리기/보내주기 · AI 기억 보조(다중 LLM) · Slack/Discord 주간 알림. ([§2-4](#2-4-2차-구현-기능-mvp-이후))
...
| 같은 폴더가 키 차이(드라이브 케이싱·구분자·호스트)로 갈라짐 | **canonical key 정규화 + 시작 시 백필 마이그레이션**으로 해결(2차, §2-1·§2-4) |
...
### 결정 필요(2차 논의)
1. 토큰 발급 UX — MVP 시드 토큰 하나 vs 대시보드 발급. *(미정)*
2. ~~transcript 요약 — 마지막 메시지 그대로 vs AI 1줄 요약.~~ → **해결:** 둘 다 제공. 마지막 요약은 상세에 그대로 표시 + **온디맨드 AI 요약**(다중 LLM, §2-4 c). 비용은 버튼 클릭 시에만 발생.
3. Gemini CLI 어댑터 추가 시점 — *미정.* (단, AI 요약은 공급자 무관 설계로 Gemini API 연결은 이미 가능.)
