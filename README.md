...
- **액션** — 카드 우측 상단의 📌 고정 / 🗄 아카이브 아이콘(상시 표시), 제목 클릭 시
  상세(스파크라인·최근 요약·완성도 수동 설정).
...
- Windows 경로의 **드라이브 문자 대소문자**(`d:\…` vs `D:\…`)는 서버 수집 단계에서
  정규화되어, 같은 폴더가 두 프로젝트로 갈라지지 않습니다(에이전트 구현·버전 무관).
