...
- **유령 리더보드** — 4가지 정렬: 🏆 가장 활발 / 👻 가장 위험한 유령 /
  💔 아까운 유령 / 🔥 모멘텀.
- **💔 아까운 유령** — 거의 완성됐는데 방치된, 잃기 아까운 유령을 우선 노출.
  유령 점수를 완성도로 가중 (`유령점수 × (0.25 + 0.75 × 완성도/100)`).
- **되살리기 / 보내주기** — 카드에서 바로 행동. **되살리기**는 `cd <경로> && claude`
  복귀 명령을 복사(아카이브였으면 복원), **보내주기**는 회고(묘비명) 한 줄과 함께 안식.
- **🤖 AI 기억 보조 (선택)** — 최근 세션 기록으로 "무엇을 하고 있었는지 + 다음 할 일"을
  요약. **공급자 무관** — OpenAI 호환 엔드포인트면 Claude·OpenAI·Gemini·Ollama 무엇이든
  설정만으로 연결. 온디맨드(버튼 누를 때만 호출).
- **🔔 Slack/Discord 알림 (선택)** — 홈에서 웹훅 URL만 붙여넣으면 주간 유령 리포트를
  전송. 채널은 URL로 자동 인식.
...
- **장애 격리** — Hook은 타임아웃·항상 `exit 0`·오프라인 큐잉으로 **Claude 동작을
  절대 막지 않음**.
- **가벼운 설치** — DB는 SQLite 파일 하나. 별도 인프라 불필요. AI·알림은 키/웹훅
  없으면 자동 비활성이라 기본 설치는 그대로 단순합니다.
...
- **정렬 탭**
  ...
  - 💔 **Most Regrettable** — 유령 등급 이상만, **완성도로 가중한** 후회 점수 높은 순
    (거의 완성됐는데 버려진 프로젝트가 위로).
  - 🔥 **Momentum** — 활성도(모멘텀) 높은 순.
- **유령 등급** (마지막 활동 이후 경과 — 카드에는 머터리얼 아이콘 + 라벨로 표시)
  | 등급 | 기준 |
  |---|---|
  | 🔥 생생함 | 3일 미만 |
  | ❄️ 식는 중 | 3 ~ 14일 |
...
- **기여도 히트맵** — 각 카드의 `HeatMap` 격자(최근 30일, 오래된 날→최근 순으로
  10칸씩 줄바꿈). 하루 작업량(턴)에 따라 5단계로 진해집니다 (GitHub 잔디밭 스타일).
- **액션** — 카드 우측 상단(상시 표시):
  - 📌 **고정** — 정렬과 무관하게 상단 고정.
  - ↩️ **되살리기** — `cd <경로> && claude` 복귀 명령을 클립보드로 복사(아카이브였으면
    복원). 유령을 다시 열어 이어서 작업할 때.
  - 👋 **보내주기** — 회고 한 줄(묘비명)을 남기고 안식(아카이브). 안식한 카드 하단에
    `🪦 회고`가 표시됩니다.
- **상세 패널** (제목 클릭) — 스파크라인 · 최근 세션 요약 · 완성도 수동 설정 ·
  **🤖 AI 기억 보조**(요약 생성/다시 생성 버튼 → "무엇을 하고 있었는지 + 다음 할 일").
- **알림 설정** — 헤더의 🔔 버튼에서 Slack/Discord 웹훅 설정 (아래 참고).

> UI 아이콘은 Google **Material Symbols**(Rounded)로 통일되어 있습니다.

---

## 🤖 AI 기억 보조 (선택)

방치된 프로젝트를 다시 열었을 때 "내가 뭘 하고 있었지?"를 떠올리게 해 줍니다.
최근 세션 기록으로 **한 줄 요약 + 다음 할 일**을 생성합니다. **온디맨드**(상세 패널의
요약 버튼을 누를 때만 호출)이라 비용이 최소화됩니다.

**공급자 무관** — OpenAI 호환 `/chat/completions` 엔드포인트면 무엇이든 `.env`에서
설정만 바꿔 연결합니다. 키가 없으면 기능은 자동 비활성(503)입니다.
...
GPH_LLM_API_KEY=             # 비우면 기능 비활성
```

> 설정 검증: `node --env-file=.env --experimental-strip-types scripts/llm-smoke.ts`
> 로 실제 공급자에 한 번 호출해 응답을 확인할 수 있습니다.

---

## 🔔 알림 — Slack / Discord (선택)

대시보드 헤더의 **🔔 알림 설정** 버튼에서 웹훅 URL만 붙여넣으면 됩니다. 채널 종류는
URL로 **자동 인식**(Slack `hooks.slack.com` / Discord `discord.com/api/webhooks`).

- **저장 / 테스트 전송 / 삭제**를 한 모달에서. "테스트 전송"으로 저장 전에 채널에 바로
  보내 확인할 수 있습니다.
- **주간 리포트** — 활성·유령·무덤 수, 💔 가장 아까운 유령 Top3, 🔥 이번 주 가장 활발.
  7일마다 자동 전송(서버 스케줄러).
- 설정은 사용자별로 DB에 저장됩니다. 웹훅이 없으면 아무것도 보내지 않습니다.
...
- **키 정규화(canonical key)** — 서버 수집 단계에서 로컬 키를 정규형으로 변환해 같은
  폴더가 갈라지지 않게 합니다(에이전트 구현·버전 무관):
  - Windows 경로는 **전체 대소문자 무시** + 구분자 통일(`\`) + 끝 슬래시 제거
    (`D:\Proj\Foo` = `d:/proj/foo/`), 호스트명 소문자화. POSIX 경로는 대소문자 유지.
  - 서버 시작 시 **백필 마이그레이션**이 과거에 갈라져 저장된 중복까지 한 프로젝트로
    병합합니다(멱등 — 정규화 도입 이전 데이터도 소급 정리).
...
- [x] 💔 아까운 유령 정렬 · 되살리기/보내주기 액션
- [x] transcript AI 요약 (다중 LLM — Claude/OpenAI/Gemini/Ollama)
- [x] Slack/Discord 주간 다이제스트 알림
